...
def correct_person_classifications(all_results):
    ...
    # Sort by Year, ID.ON, sentence_id, and token_id to ensure proper order
    df = df.sort_values(['Year', 'ID.ON', 'sentence_id', 'token_id'])
    
    window_size = 4  # Number of entries to check above and below
    ...
    for i in range(len(df)):
        if df.iloc[i]['entity_type'] != 'PERSON':
            ...
            # Count PERSONs in surrounding entries
            person_count = sum(surrounding['entity_type'] == 'PERSON')
            
            # If at least 4 surrounding entries are PERSON, change current entry to PERSON
            if person_count >= 8:
